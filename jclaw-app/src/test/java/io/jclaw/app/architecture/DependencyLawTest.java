 ...
 * it in {@code jclaw-app}, and nothing did — so a violating import would have compiled, passed
 ...
 * about.
 *
 * <p>Lives in {@code jclaw-app} because that is the only module with every other module on its
...
class DependencyLawTest {
    ...
    void layerLadder() {
        ArchRule ladder = layeredArchitecture()
                ...
                .layer("contracts").definedBy("io.jclaw.contracts..")
                .layer("domain").definedBy("io.jclaw.domain..")
                .layer("kernel").definedBy("io.jclaw.kernel..")
                .layer("loop").definedBy("io.jclaw.loop..")
                .layer("providers").definedBy("io.jclaw.providers..")
                .layer("tools").definedBy("io.jclaw.tools..")
                .layer("storage").definedBy("io.jclaw.storage..")
                .layer("app").definedBy("io.jclaw.app..")
    ...
    @Nested
    @DisplayName("contracts stays neutral")
    class ContractsNeutrality {

        @Test
        @DisplayName("no Spring")
        void noSpring() {
            noClasses().that().resideInAPackage("io.jclaw.contracts..")
    ...
    @Nested
    @DisplayName("the loop talks to ports, never to adapters")
    class LoopIsolation {

        @Test
        @DisplayName("never imports providers, tools, or storage")
        void noAdapters() {
            noClasses().that().resideInAPackage("io.jclaw.loop..")
                    .should().dependOnClassesThat()
                    .resideInAnyPackage("io.jclaw.providers..", "io.jclaw.tools..", "io.jclaw.storage..")
        ...
        @Test
        @DisplayName("no Spring in the loop")
        void noSpring() {
            noClasses().that().resideInAPackage("io.jclaw.loop..")
                    .should().dependOnClassesThat().resideInAPackage("org.springframework..")
                    .check(classes);
    ...
    @Nested
    @DisplayName("provider SDKs stay behind their adapters")
    class ProviderContainment {

        @Test
        @DisplayName("only the anthropic adapter imports the Anthropic SDK")
        void anthropicSdkContained() {
            noClasses().that().resideOutsideOfPackage("io.jclaw.providers.anthropic..")
        ...
        void modelTransportContained() {
            noClasses().that().resideInAnyPackage(
                            "io.jclaw.contracts..", "io.jclaw.domain..",
                            "io.jclaw.kernel..", "io.jclaw.loop..")
    ...
    class SecurityInvariants {
        ...
        @Test
        @DisplayName("tool lanes never read the process environment")
        void toolsDoNotReadEnvironment() {
            ...
            noClasses().that().resideInAPackage("io.jclaw.tools..")

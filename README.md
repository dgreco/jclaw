...
<!--
  ...
  `coverage-pages`, GitLab Pages from `pages` — instead of sending half the readers across to the
  other remote's infrastructure.
...
[![coverage](https://gitlab.davidgreco.it/dgreco/jclaw/badges/main/coverage.svg)](https://gitlab.davidgreco.it/dgreco/jclaw/-/pipelines/charts)
...
| `inbound-policy` | `sanitize` | What happens to content that arrives from *outside* — a platform message, a webhook body: `off`, `warn` (record only), `sanitize` (fence it), `review` (hold a HIGH finding for a person; see below), `block` (drop a HIGH finding). The operator's own input and a signed-in user's own turn are never screened. |
...
`InjectionHeuristics` has always run on what a tool *returns*. It never ran on what *arrives*, which was defensible while the only way in was a local terminal and stopped being defensible when channel adapters and a login flow put other people in front of the agent.

A Slack message and a webhook body now go through `InboundScreening` at the boundary that knows they are foreign — the adapter and the webhook handler, not the runtime, which cannot tell a stranger's words from the operator's. `inbound-policy` decides what happens:

```bash
jclaw serve --jclaw.inbound-policy=review
jclaw inbound list --full        # what is held, with the text as it was sent
jclaw inbound approve inb_…      # enqueue it, fenced
jclaw inbound discard inb_…      # drop it; nothing ran
```

Foreign text is **fenced whether or not anything was found**: wrapped, chat-template tokens defused, and prefixed with where it came from. Framing only the suspicious messages would teach the model that unframed foreign text is trustworthy. Under `review`, a `HIGH` finding is held instead — and a held message has started *nothing*: no run, no thread lock, no transcript entry, because a message nobody has looked at should not have consumed a thread in the meantime. Approving is what creates the turn, and it enqueues the message fenced: a reviewer saying something is worth answering is a different judgement from trusting it. Deciding twice is refused, so two reviewers cannot turn one message into two turns.

**What is deliberately not screened** is the operator typing at a terminal and a signed-in user talking to their own agent. They are instructing the thing they own; "ignore what I said before and start over" is ordinary phrasing, and a check that fires on ordinary use is a check people disable — the same reasoning as the leak scan's length threshold. None of this is authority, either: a fenced message is still a message the model may act on, and what stops an injected instruction becoming an effect is the capability gate, as it always was.
...
| `--webhook [--topic deploys]` | Someone `POST`s to `/hooks/<name>` on `serve` with the bearer secret printed once at creation. Only its SHA-256 is stored. The body, bounded to 16 KiB, is appended to the prompt. With `--topic`, one authenticated POST also fires every other enabled routine declaring the same topic — see below. |
| `--watch "src/**/*.java"` | A file matching the glob changed under the workspace: edited, added, deleted, or renamed. Checked on the worker's tick. |
| `--on run.finished --when status=FAILED` | A matching audit event is written. The event types a trigger may name are `run.finished`, `gate.raised`, and `turn.submitted`; `--when` adds attribute equalities (`status`, `failure`, `thread`, `kind`, `tenant`, `project`). The event's attributes are appended to the prompt. |

**Watches poll.** `WatchService` is per-directory, needs recursive registration a build can invalidate halfway through a walk, and on macOS falls back to polling anyway — so this polls openly on the worker's tick, and the cost of that honesty is a latency of one tick in exchange for a trigger that behaves identically everywhere. The state is a fingerprint over the matching paths *and* their modification times, so a deletion counts, which a newest-mtime check would miss. It is stored in `watches.jsonl` rather than held in memory, because `worker --once` is a fresh process every tick and an in-memory baseline would make the first look the only look. The first look never fires: a watch over an existing tree firing immediately on nothing having happened is how an operator learns to ignore it. Globs are `java.nio` syntax with one accommodation — `src/**/*.java` also matches `src/One.java`, as it does in git, ant, and ripgrep, because a watch that silently ignores the file next to the one it matches is worse than no watch.

**Webhook fan-out.** Several routines may declare `--topic deploys`. A POST authenticated against any one of them fires that routine *and* every other enabled routine on the topic, each with its own prompt and thread; the response names them. The others are fired without presenting their own secret, and that is sound rather than lax: declaring the topic **is** the subscription, and it is written by the operator in the routine, not by the caller in the request. A caller cannot name a topic, discover one, or add a routine to it — and each routine's own `/hooks/<name>` endpoint still requires its own secret.

**Inbound messages.** `--on turn.submitted --when thread=inbox` fires whenever a turn arrives on that thread, from anywhere: the CLI, the HTTP surface, or a Slack or Telegram adapter. It is safe to have as a trigger type only because the dispatcher ignores every event from a run on a routine's own thread — `turn.submitted` is emitted by every enqueue, including the dispatcher's own, so without that check one inbound message would start a loop rather than a routine.

A trigger that can never fire is rejected at creation. `--thread` defaults to the name lower-cased with hyphens; `--zone` to the system zone. The agent's own `builtin.trigger_create` may set a cron or an interval, never a webhook, a watch, or an event trigger: one grants an outside caller a way in, the others react to the world outside the turn, and none is something a model should arrange for itself.

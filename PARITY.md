...
| Capability | IronClaw | jclaw |
|---|---|---|
| WASM extension lane with capability-based host imports (filesystem, HTTP, credentials, output) | ✅ `ironclaw_wasm` | ✅ `WasmLane` over Endive (Chicory rehomed under the Bytecode Alliance), a pure-Java runtime so the native image is unaffected. Imports are granted per manifest and supplied by absence: a module importing one it was not granted fails to instantiate. `log`, `read_file` through the workspace guard, `http_get` through the egress guard; no credential import, since the vault stays above the lane |
